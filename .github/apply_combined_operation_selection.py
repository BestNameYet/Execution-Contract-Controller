...
path = Path('runtime-source/execution_contract_controller.py')
...
indirect = '''INDIRECT_WORK_DEFINITION = {
...
'''
insert_marker = '# Every static string literal in this exact controller source receives a compact\n'
if 'INDIRECT_WORK_DEFINITION = {' not in source:
    source = source.replace(insert_marker, indirect + '\n' + insert_marker, 1)

source = replace_function(source, 'realization_output_schema', '''def realization_output_schema() -> dict[str, Any]:
...
source = replace_function(source, 'decision_context', '''def decision_context(state: dict[str, Any], pid: str, kind: str) -> dict[str, Any]:
    dependencies = []
...
source = replace_function(source, 'realization_request', '''def realization_request(state: dict[str, Any]) -> dict[str, Any]:
    if all_done(state):
...
source = replace_function(source, 'handle_realization', '''def _authorize_classified_operation(state: dict[str, Any], op: dict[str, Any], r: dict[str, Any]) -> dict[str, Any]:
...
source = replace_function(source, 'handle_admissibility', '''def handle_admissibility(inp: dict[str, Any]) -> dict[str, Any]:
    p = verify(req_dict(inp.get("payload"), "payload"))
    state = deepcopy(p["state"])
