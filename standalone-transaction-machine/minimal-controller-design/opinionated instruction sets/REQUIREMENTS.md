...
| OIS-024 | In this minimal opinionated instruction set, the first question sent to the model must request the current user prompt. | MET | `initial.py`: `main()` first constructs `user_prompt_request` with the instruction `Return the current user prompt exactly...` and immediately calls `emit(user_prompt_request)` before constructing or emitting `question_request` or any model-generated interrogation question. |
