...
| OIS-017 | `initial.py` must validate each stdin response against the exact expected shape for its current stage. | MET | `initial.py`: `validate_desired_state()`, `validate_q1()`, `validate_answer()`, and `validate_script()` require exact key sets and expected value types; `receive_valid()` applies the stage validator. |
